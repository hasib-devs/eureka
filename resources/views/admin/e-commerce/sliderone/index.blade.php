...
@section('content')
    ...
    <section class="mb-6">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900">Slider One</h1>
                <p class="mt-1 text-sm text-slate-500">Manage the secondary slider images</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ routeHelper('sliderone/create') }}"
                    ...
                    <i class="fas fa-plus text-xs"></i> Add Slider One
                </a>
                <ol class="flex items-center gap-1 text-sm text-slate-400">
                    <li><a href="{{ routeHelper('dashboard') }}" class="transition-colors hover:text-primary">Home</a></li>
                    <li><i class="fas fa-chevron-right text-[9px]"></i></li>
                    <li class="font-medium text-slate-600">Slider One</li>
                </ol>
            </div>
        </div>
    </section>
    ...
    <section class="mb-6">
        <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 px-5 py-4">
                <h2 class="text-base font-semibold text-slate-900">All Slider One</h2>
                <span class="inline-flex items-center rounded-full bg-primary/10 px-2.5 py-1 text-xs font-semibold text-primary ring-1 ring-inset ring-primary/20">
                    {{ count($new_sliders_one) }} {{ Str::plural('slider', count($new_sliders_one)) }}
                </span>
            </div>
                            ...
                            <tr class="group transition-colors hover:bg-slate-50/80">
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <a href="{{ routeHelper('sliderone/' . $data->nsid) }}"
                                        ...
                                        <a href="javascript:void(0)" id="deleteData" data-id="{{ $data->nsid }}"
                                            title="Delete slider"
                            ...
                            <tr>
                                <td colspan="5" class="px-5 py-20 text-center">
                                    ...
                                    <p class="mt-1 text-sm text-slate-500">
                                        Get started by
                                        <a href="{{ routeHelper('sliderone/create') }}" class="text-primary hover:underline">adding your first slider</a>.
                                    </p>
                                </td>
                            </tr>
                        @endforelse
