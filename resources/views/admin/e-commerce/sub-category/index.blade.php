...
@section('content')
    ...
    <section class="mb-6">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900">Sub Categories</h1>
                <p class="mt-1 text-sm text-slate-500">Manage sub categories nested under each category</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ routeHelper('sub-category/create') }}"
                    ...
                    <i class="fas fa-plus text-xs"></i> Add Sub Category
                </a>
                <ol class="flex items-center gap-1 text-sm text-slate-400">
                    <li><a href="{{ routeHelper('dashboard') }}" class="transition-colors hover:text-primary">Home</a></li>
                    <li><i class="fas fa-chevron-right text-[9px]"></i></li>
                    <li class="font-medium text-slate-600">Sub Categories</li>
                </ol>
            </div>
        </div>
    </section>
    ...
    <section class="mb-6">
        <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 px-4 py-4">
                <h2 class="text-base font-semibold text-slate-900">All Sub Categories</h2>
                <span class="inline-flex items-center rounded-full bg-primary/10 px-2.5 py-1 text-xs font-semibold text-primary ring-1 ring-inset ring-primary/20">
                    {{ count($sub_categories) }} {{ Str::plural('sub category', count($sub_categories)) }}
                </span>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full min-w-[860px] text-left text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($sub_categories as $key => $data)
                            <tr class="group transition-colors hover:bg-slate-50/80">
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        @else
                                            <img src="/uploads/sub category/{{ $data->cover_photo }}" alt="Cover Photo" class="h-10 w-10 rounded-lg object-cover ring-1 ring-slate-200">
                                        @endif
                                        <div>
                                ...
                                <td class="px-4 py-4 text-slate-500">{{ $data->category->name ?? '' }}</td>
                                <td class="px-4 py-4">
                                    @if ($data->status)
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <a href="{{ route('subCategory.product', ['slug' => $data->slug]) }}"
                                            title="View products"
                                        ...
                                        <a href="{{ routeHelper('sub-category/' . $data->id . '/edit') }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-5 py-20 text-center">
                                    ...
                                    <p class="font-semibold text-slate-700">No sub categories found</p>
                                    <p class="mt-1 text-sm text-slate-500">
                                        Get started by
                                        <a href="{{ routeHelper('sub-category/create') }}" class="text-primary hover:underline">adding your first sub category</a>.
                                    </p>
                                </td>
                            </tr>
                        @endforelse
