...
@section('content')
    ...
    <section class="mb-6">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900">Sizes</h1>
                <p class="mt-1 text-sm text-slate-500">Manage the size options available for products</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ routeHelper('size/create') }}"
                ...
                <ol class="flex items-center gap-1 text-sm text-slate-400">
                    ...
                    <li class="font-medium text-slate-600">Sizes</li>
                </ol>
            </div>
        </div>
    </section>
    ...
    <section class="mb-6">
        <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 px-5 py-4">
                <h2 class="text-base font-semibold text-slate-900">All Sizes</h2>
                <span class="inline-flex items-center rounded-full bg-primary/10 px-2.5 py-1 text-xs font-semibold text-primary ring-1 ring-inset ring-primary/20">
                    {{ count($sizes) }} {{ Str::plural('size', count($sizes)) }}
                </span>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($sizes as $key => $data)
                            <tr class="group transition-colors hover:bg-slate-50/80">
                                ...
                                <td class="px-4 py-4 text-slate-500">{{ $data->description }}</td>
                                <td class="px-4 py-4">
                                    @if ($data->status)
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <a href="{{ routeHelper('size/' . $data->id . '/edit') }}"
                                            title="Edit size"
                            ...
                            <tr>
                                <td colspan="5" class="px-5 py-20 text-center">
                                    <div class="mx-auto mb-4 grid h-14 w-14 place-items-center rounded-2xl bg-slate-50 ring-1 ring-slate-200">
                                        <i class="fas fa-ruler-combined text-xl text-slate-300"></i>
                                    </div>
                                    <p class="font-semibold text-slate-700">No sizes found</p>
                                    <p class="mt-1 text-sm text-slate-500">
                                        Get started by
                                        <a href="{{ routeHelper('size/create') }}" class="text-primary hover:underline">adding your first size</a>.
                                    </p>
                                </td>
                            </tr>
                        @endforelse
