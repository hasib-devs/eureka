...
@section('content')
    ...
    <section class="mb-6">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900">Processing Orders</h1>
                <p class="mt-1 text-sm text-slate-500">Confirmed orders currently being prepared</p>
            </div>
            <div class="flex items-center gap-3">
                <ol class="flex items-center gap-1 text-sm text-slate-400">
                    <li><a href="{{ routeHelper('dashboard') }}" class="transition-colors hover:text-primary">Home</a></li>
                    <li><i class="fas fa-chevron-right text-[9px]"></i></li>
                    <li class="font-medium text-slate-600">Processing Order List</li>
                </ol>
            </div>
        </div>
    </section>
    ...
    <section class="mb-6">
        <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 px-4 py-4">
                <h2 class="text-base font-semibold text-slate-900">Processing Order List</h2>
                            ...
                            <tr class="group transition-colors hover:bg-slate-50/80">
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        <a href="{{ routeHelper('order/status/shipping/' . $data->id) }}" id="btnShipping"
                                            title="Shipping" onclick="return confirm('Are you sure Shipping this order?')"
                                            class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 shadow-sm transition-all hover:-translate-y-px hover:border-primary hover:bg-primary-50 hover:text-primary hover:shadow">
                                            <i class="fas fa-plane"></i>
                                        </a>
                                        <a href="{{ routeHelper('order/status/cancel/' . $data->id) }}"
                                            title="Cancel" onclick="return confirm('Are you sure cancel this order?')"
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="px-5 py-20 text-center">
                                    ...
                                    <p class="font-semibold text-slate-700">No processing orders found</p>
                                    <p class="mt-1 text-sm text-slate-500">Orders moved to processing will show up here.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
