    ...
    <section class="mb-6">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500">Manage the top-level product categories of your store</p>
            </div>
            <div class="flex items-center gap-3">
                ...
                <ol class="flex items-center gap-1 text-sm text-slate-400">
                    ...
                    <li class="font-medium text-slate-600">Categories</li>
                </ol>
            </div>
        </div>
    </section>
                ...
                <table class="w-full min-w-[860px] text-left text-sm">
                    <thead>
                        <tr class="border-b border-slate-200 bg-slate-50/80 text-[11px] font-semibold uppercase tracking-wider text-slate-400">
                            ...
                            <th class="px-4 py-3.5">Homepage</th>
                            ...
                            <tr class="group transition-colors hover:bg-slate-50/80">
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        @else
                                            <img src="/uploads/category/{{ $data->cover_photo }}" alt="Cover Photo" class="h-10 w-10 rounded-lg object-cover ring-1 ring-slate-200">
                                        @endif
                                        <div>
                                ...
                                <td class="px-4 py-4 text-slate-500">{{ Str::words($data->description, 5, '...') }}</td>
                                <td class="px-4 py-4">
                                    @if ($data->status)
                                ...
                                <td class="px-4 py-4">
                                    <x-ui.badge variant="{{ $data->is_shown_on_homepage ? 'success' : 'danger' }}" dot>
                                        {{ $data->is_shown_on_homepage ? 'Yes' : 'No' }}
                                    </x-ui.badge>
                                </td>
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        <a href="{{ routeHelper('category/' . $data->id) }}"
                                            title="View category"
                                            class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 shadow-sm transition-all hover:-translate-y-px hover:border-primary hover:bg-primary-50 hover:text-primary hover:shadow">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ routeHelper('category/' . $data->id . '/edit') }}"
                                            title="Edit category"
                                            class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 shadow-sm transition-all hover:-translate-y-px hover:border-primary hover:bg-primary-50 hover:text-primary hover:shadow">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-5 py-20 text-center">
                                    ...
                                    <p class="font-semibold text-slate-700">No categories found</p>
                                    <p class="mt-1 text-sm text-slate-500">
                                        Get started by
                                        <a href="{{ routeHelper('category/create') }}" class="text-primary hover:underline">adding your first category</a>.
                                    </p>
                                </td>
                            </tr>
                        @endforelse
