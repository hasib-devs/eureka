...
@section('content')
    ...
    <section class="mb-6">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900">Colors</h1>
                <p class="mt-1 text-sm text-slate-500">Manage the color options available for products</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ routeHelper('color/create') }}"
    ...
    <section class="mb-6">
        <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 px-5 py-4">
                <h2 class="text-base font-semibold text-slate-900">All Colors</h2>
                <span class="inline-flex items-center rounded-full bg-primary/10 px-2.5 py-1 text-xs font-semibold text-primary ring-1 ring-inset ring-primary/20">
                    {{ count($colors) }} {{ Str::plural('color', count($colors)) }}
                </span>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($colors as $key => $data)
                            <tr class="group transition-colors hover:bg-slate-50/80">
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center gap-3">
                                        <span class="h-6 w-6 shrink-0 rounded-full ring-2 ring-white shadow" style="background: {{ $data->code }}"></span>
                                        <p class="font-semibold text-slate-800">{{ $data->name }}</p>
                                    </div>
                                </td>
                                <td class="px-4 py-4 text-slate-500">{{ Str::words($data->description, 5, '...') }}</td>
                                <td class="px-4 py-4 uppercase tabular-nums text-slate-600">{{ $data->code }}</td>
                                <td class="px-4 py-4">
                                    @if ($data->status)
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <a href="{{ routeHelper('color/' . $data->id) }}"
                                            title="More details"
                                        ...
                                        <a href="{{ routeHelper('color/' . $data->id . '/edit') }}"
                                            title="Edit color"
                                            class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 shadow-sm transition-all hover:-translate-y-px hover:border-primary hover:bg-primary-50 hover:text-primary hover:shadow">
                            ...
                            <tr>
                                <td colspan="6" class="px-5 py-20 text-center">
                                    <div class="mx-auto mb-4 grid h-14 w-14 place-items-center rounded-2xl bg-slate-50 ring-1 ring-slate-200">
                                        <i class="fas fa-palette text-xl text-slate-300"></i>
                                    </div>
                                    <p class="font-semibold text-slate-700">No colors found</p>
                                    <p class="mt-1 text-sm text-slate-500">
                                        Get started by
                                        <a href="{{ routeHelper('color/create') }}" class="text-primary hover:underline">adding your first color</a>.
                                    </p>
                                </td>
                            </tr>
                        @endforelse
