...
.hamburger .bars .b1 { top: 0; }
.hamburger .bars .b2 { top: 8px; }
.hamburger .bars .b3 { top: 16px; }
...
.hamburger.is-active .bars .b3 {
    ...
    @media (max-width: 768px) {
        ...
        .hdr-center { flex: 1; justify-content: flex-start; }
        .hdr-right { gap: 8px; }
...
/* Logo shrink */
.main-header.is-sticky .logo-link img {
    height: 66px !important;   /* 🔥 adjust 10-14px smaller */
    max-height: 70px;
...
<div class="main-header">
    <div class="header-row container">
        ...
        <div class="hdr-right">
            ...
            {{-- NOTE: component listens for [data-open-mobile-sidebar] to open --}}
            <button type="button" id="hamburgerBtn" class="hamburger mobile-only" aria-label="Open menu" aria-expanded="false" data-open-mobile-sidebar>
...
<div id="mobileSearchOverlay" class="mobile-search-overlay" aria-hidden="true" role="dialog" aria-label="Mobile search" style="display:none;">
    <div class="mobile-search-panel" role="search" style="max-width:880px; margin:6vh auto 0; padding:18px; background:#fff; border-radius:10px; box-shadow:0 18px 40px rgba(2,6,23,0.2); position:relative;">
        <form action="{{ route('product.search') }}" method="GET" style="display:flex; width:100%;">
            <input id="mobileSearchInput" name="search" type="search" placeholder="Search products..." aria-label="Search products" autofocus style="flex:1; padding:12px 14px; border-radius:8px; border:1px solid #e6e9ef;">
            <button type="submit" aria-label="Search" style="background:var(--bg-1, #2E6CB6); border-radius:12px; padding:12px 14px; border:none; margin-left:8px; color:#fff;">
                <i class="fal fa-search" style="font-size:18px;"></i>
            </button>
        </form>

        <button id="mobileSearchClose" aria-label="Close search" style="position:absolute; top:10px; right:10px; background:transparent; border:0; font-size:18px; color:#6b7280;">
...
{{-- Cart Notification --}}
<div class="cart-notification" id="cartNotification" style="display:none; position:fixed; bottom:18px; right:18px; background:#2e7d32; color:#fff; padding:10px 14px; border-radius:8px; box-shadow:0 8px 24px rgba(0,0,0,0.18); z-index:14000;">
...
if (hamburgerBtn) {
    ...
    (function(){
        ...
        function openSearch() {
            ...
            document.dispatchEvent(new CustomEvent('mobile-sidebar-control', { detail: { action: 'close' } }));
        }
