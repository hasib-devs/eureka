...
@push('meta')
<meta name='description' content="{{ $category->name }} Products"/>
...
<style>
    ...
    /* ── PRODUCT GRID — mirrors the Shop page: clean vertical layout, 4 → 3 → 1 ── */
    .sp-products {
        width: 100%;
        background: #fff;
        padding: 40px 0 66px;
    ...
    @media (max-width: 992px) {
        .sp-products .lux-product-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); }
    }
    @media (max-width: 768px) {
        .sp-products .lux-product-grid { grid-template-columns: repeat(1, minmax(0, 1fr)); }
    }
...
<div class="sp-hero">
    ...
    <h1 class="sp-hero-title">{{ $category->name }}</h1>
</div>

{{-- ====== PRODUCT GRID (clean, mirrors the Shop page — product cards only) ====== --}}
<section class="sp-products">
    <div class="px-4">
        <div class="lux-product-grid">
            @forelse ($products as $product)
                <x-lux-product-card :product="$product" :category="$category->name" />
            @empty
