...
class adsController extends Controller
{
    ...
    {
        Unproduct::where('id', $id)->where('user_id', auth()->id())->firstOrFail()->delete();
    ...
    {
        $product = Unproduct::where('id', $id)->where('user_id', auth()->id())->firstOrFail();
    ...
    {
        $product = Unproduct::where('id', $request->power)->where('user_id', auth()->id())->firstOrFail();
        $this->validate($request, [
            'title' => 'required|string|max:255',
            'location' => 'required|string',
            'contact' => 'required|string',
    ...
    public function all(Request $request)
    {
        ...
        $products = Unproduct::where('status', 1)
