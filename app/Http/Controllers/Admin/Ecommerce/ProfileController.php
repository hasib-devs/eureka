...
class ProfileController extends Controller
{
    ...
    // Lightweight profile-picture upload (used by the sidebar avatar picker)
    public function updateAvatar(Request $request)
    {
        ...
        $imageName = Carbon::now()->toDateString().'-'.uniqid().'.'.$avatar->extension();

        if ($user->avatar && file_exists(public_path('uploads/admin/'.$user->avatar))) {
            unlink(public_path('uploads/admin/'.$user->avatar));
        }
        if (! file_exists(public_path('uploads/admin'))) {
            mkdir(public_path('uploads/admin'), 0777, true);
        }
