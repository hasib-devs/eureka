...
class SettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->type == 1) {
            ...
            Setting::updateOrCreate(['name' => 'pinterest'], ['value' => $request->get('pinterest')]);
            Setting::updateOrCreate(['name' => 'tiktok'], ['value' => $request->get('tiktok')]);

            // Product page: accordion copy + trust badges shown under the buy buttons.
            Setting::updateOrCreate(['name' => 'PRODUCT_SHIPPING_TEXT'], ['value' => $request->get('PRODUCT_SHIPPING_TEXT')]);
            Setting::updateOrCreate(['name' => 'PRODUCT_WARRANTY_TEXT'], ['value' => $request->get('PRODUCT_WARRANTY_TEXT')]);
            Setting::updateOrCreate(['name' => 'TRUST_BADGE_1'], ['value' => $request->get('TRUST_BADGE_1')]);
            Setting::updateOrCreate(['name' => 'TRUST_BADGE_2'], ['value' => $request->get('TRUST_BADGE_2')]);
            Setting::updateOrCreate(['name' => 'TRUST_BADGE_3'], ['value' => $request->get('TRUST_BADGE_3')]);

            notify()->success('Site information successfully updated', 'Success');
