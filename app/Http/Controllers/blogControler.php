...
{
    /**
     * Allow the action only for a platform admin (role_id 1) or the blog's owner.
     * These blog mutations are wired to both the admin and the account route groups,
     * so admins must retain moderation over any blog while account users are
     * restricted to their own.
     */
    private function authorizeBlog(Blog $blog): void
    {
        if (auth()->user()->role_id != 1 && $blog->user_id != auth()->id()) {
            abort(403);
        }
    }
    ...
    {
        $blog = Blog::findOrFail($request->power);
        $this->authorizeBlog($blog);
    ...
    {
        $blogs = Blog::where('user_id', auth()->id())->get();
        $eblog = Blog::where('id', $id)->where('user_id', auth()->id())->firstOrFail();
        $categories = Category::all();
